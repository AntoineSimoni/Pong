...
class Pong:  # la classe gerant la balle de pong
    def __init__(self):  # le constructeur de la balle de pong
        ...
        self.r1 = Raquette(30, Y / 2 - self.ty / 2, "<z>",
                           "<s>")  # on recup les deux raquettes pour plus tard check la hitbox
    ...
    def deplacer(self):
        global PLAY, x, y, SPEED, SAVESPEED
        if PLAY:
            ...
            tk.title("J1 : " + str(self.J1) + "/" + str(point) + "  |||  J2 : " + str(self.J2) + "/" + str(
                point) + "                                      Appuyez sur entrée pour Démarer/Mettre en pause")  # on change le titre de la fenetre pour servir de score
...
def jeu():
    ...
    terrain.pack()
    jeux = Pong()
    tk.title("J1 : " + str(jeux.J1) + "/" + str(point) + "  |||  J2 : " + str(jeux.J2) + "/" + str(
        point) + "                                      Appuyez sur entrée pour Démarer/Mettre en pause")
    tk.mainloop()
...
def menu_fin():
    ...
    if jeux.J1 == point:
        lab = Label(win, text="J1 WIN ! Il y a " + str(jeux.J1) + " à " + str(jeux.J2) + " pour J1" + " en " + str(
    ...
    if jeux.J2 == point:
        lab = Label(win, text="J2 WIN ! Il y a " + str(jeux.J2) + " à " + str(jeux.J1) + " pour J2" + " en " + str(
            minutes) + " min " + str(secondes) + " sec ")
        lab.grid(row=1, column=1, columnspan=2, padx=10, pady=10)
        fin_rejouer = Button(win, text="Rejouer", command=rejouer)
        fin_rejouer.grid(row=2, column=2, padx=10, pady=10)
...
def parametre():
    ...
    def valider():
        global point, SPEED, COLORsraq, COLORsball, SAVESPEED, INC_VITESSE, INC
        point = score.get()
        SPEED = vitesse.get()
        INC = INC_vitesse.get()
        SAVESPEED = vitesse.get()
    ...
    INC_VITESSE = Label(parametre, text="Incrémentation de la vitesse de la balle :")
    INC_VITESSE.grid(row=4, column=1, padx=5, pady=5)
    INC_vitesse = IntVar()
    INC_VITESSE = Entry(parametre, textvariable=INC_vitesse, width=5)
    INC_VITESSE.grid(row=4, column=2, padx=10)

    COULEURraq = Label(parametre, text="Couleur raquette : 0=blanc, 1=rouge , 2=vert, 3=violet ")
    COULEURraq.grid(row=5, column=1, padx=5, pady=5)
    couleurraq = IntVar()
    COULEURraq = Entry(parametre, textvariable=couleurraq, width=5)
    COULEURraq.grid(row=5, column=2, padx=10)

    COULEURball = Label(parametre, text="Couleur balle : 0=blanc, 1=rouge , 2=vert, 3=violet ")
    COULEURball.grid(row=6, column=1, padx=5, pady=5)
    couleurball = IntVar()
    COULEURball = Entry(parametre, textvariable=couleurball, width=5)
    COULEURball.grid(row=6, column=2, padx=10)
